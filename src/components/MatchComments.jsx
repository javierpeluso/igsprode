...
function Avatar({ user }) {
  if (user.photoURL) {
    return <img src={user.photoURL} alt={user.displayName} className="comment-avatar" referrerPolicy="no-referrer" />;
  }
...
export default function MatchComments({ matchId, currentUser }) {
  ...
  useEffect(() => {
    if (!expanded) return;
    const ref = query(
      collection(db, 'comments', matchId, 'messages'),
  ...
  return (
    <div className="match-comments">
      <button className="btn-comments-toggle" onClick={() => setExpanded(v => !v)}>
        💬 Comentarios {comments.length > 0 && !expanded ? `(${comments.length})` : ''} {expanded ? '▲' : '▼'}
      </button>
              ...
              <div key={c.id} className={`comment-row ${c.userId === currentUser.uid ? 'is-me' : ''}`}>
                ...
                <div className="comment-bubble">
                  <div className="comment-meta">
                    ...
                    <span className="comment-time">{timeAgo(c.createdAt?.toMillis())}</span>
                  </div>
